...
for file_idx in range(10):
    ...
    net_source = torch.nn.Sequential(prenet, net, binary4)
    checkpoint = torch.load('./checkpoint/mnist_densenet121_ver2/mnist_task'+str(source+20)+'.t'+str(file_idx))
    ...
    net_target = torch.nn.Sequential(prenet, net, binary)
    checkpoint = torch.load('./checkpoint/mnist_densenet121_ver2/mnist_task'+str(target+20)+'.t'+str(file_idx))
    ...
    # tensor_target[:5,:] = fisher_matrix_source[name[364]]
    # fisher_matrix_source[name[364]] = tensor_target.cuda()
    
    # tensor_target = torch.zeros(10)
    # tensor_target[:5] = fisher_matrix_source[name[365]]
    # fisher_matrix_source[name[365]] = tensor_target.cuda()
    
    
    tensor_target = torch.zeros(10, 10)
    tensor_target[:2,:] = fisher_matrix_target[name[364]]
    fisher_matrix_target[name[364]] = tensor_target.cuda()
    
    tensor_target = torch.zeros(10)
    tensor_target[:2] = fisher_matrix_target[name[365]]
    fisher_matrix_target[name[365]] = tensor_target.cuda()
